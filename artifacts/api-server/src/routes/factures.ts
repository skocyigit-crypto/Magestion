...
import { db, facturesTable } from "@magestion/db";
...
facturesRouter.patch("/:id", async (req, res) => {
  ...
  const { objet, montantHt, tauxTva, dateEcheance, active } = parsed.data;
  const touchesLockedFields = objet !== undefined || montantHt !== undefined || tauxTva !== undefined || dateEcheance !== undefined;
